...
const SpotifyAnalysisTile: React.FC<SpotifyAnalysisTileProps> = ({
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-black/55 p-3 backdrop-blur-md sm:p-4">
          <div ref={modalRef} className="mx-auto flex h-[calc(100vh-1.5rem)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0f1115] text-white shadow-[0_30px_120px_rgba(0,0,0,0.55)] sm:h-[calc(100vh-2rem)]">
            <div className="border-b border-white/10 bg-gradient-to-r from-[#121521] via-[#0f1115] to-[#121521] px-5 py-4 sm:px-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="rounded-full border border-white/10 bg-white/5 p-2 text-white/70 transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/10 hover:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  { label: 'Madonna tracks', value: madonnaTracks.length.toString() }
                ].map(item => (
                  <div key={item.label} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <p className="text-[10px] uppercase tracking-[0.25em] text-white/45">{item.label}</p>
                ...
                {(['personas', 'galaxy', 'comparison'] as const).map(tab => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex items-start gap-3 rounded-xl border px-4 py-3 text-left transition-all duration-200 ${active ? 'border-cyan-400/40 bg-cyan-400/10 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]' : 'border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/8'}`}
                    >
                      <span className={`mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-lg ${active ? 'bg-cyan-400/20 text-cyan-200' : 'bg-white/5 text-white/60'}`}>
                        <Icon className="h-4 w-4" />
                      </span>
                      <span className="min-w-0">
                        <span className="block text-sm font-semibold text-white">{meta.title}</span>
                        <span className="mt-1 block text-xs leading-relaxed text-white/55">{meta.blurb}</span>
                      </span>
                    </button>
                  );
                })}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">
                ...
                <section className="space-y-5">
                  <div className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Persona Finder</p>
                          <h3 className="mt-2 text-xl font-semibold text-white">AI groups tracks by sonic behavior</h3>
                        ...
                        <div className="hidden rounded-full border border-white/10 bg-white/5 p-3 text-cyan-200 md:block">
                      ...
                      <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        {clusters.map(cluster => {
                          ...
                          const trackCount = tracks.filter(track => track.cluster === cluster.cluster).length;
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                const dominantTrackIndex = tracks.findIndex(track => track.cluster === cluster.cluster);
                                if (dominantTrackIndex >= 0) {
                                  setSelectedTrackIndex(dominantTrackIndex);
                                }
                              }}
                              className={`group rounded-2xl border p-4 text-left transition-all duration-200 ${isSelected ? 'border-white/30 bg-white/10 shadow-[0_0_0_1px_rgba(255,255,255,0.06)]' : 'border-white/10 bg-[#121521]/70 hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/8'}`}
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  ...
                                  <p className="mt-2 text-xs leading-5 text-white/60">{meta.description}</p>
                                </div>
                                <span className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[10px] uppercase tracking-[0.25em] text-white/60">
                                  {trackCount}
                                </span>
                              </div>
                                ...
                                {[
                                  ...
                                  <div key={metric.label}>
                                    <div className="mb-1 flex items-center justify-between text-white/60">
                                      ...
                                      <span className="font-medium text-white">{Math.round(metric.value * 100)}%</span>
                                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Current persona</p>
                        <div className="mt-3 flex items-center gap-3">
                          <span className="h-4 w-4 rounded-full" style={{ backgroundColor: selectedClusterMeta.color }} />
                          <h4 className="text-lg font-semibold text-white">{selectedClusterMeta.name}</h4>
                        </div>
                        <p className="mt-2 text-sm leading-6 text-white/65">{selectedClusterMeta.description}</p>

                        <div className="mt-4 rounded-xl border border-white/10 bg-[#0b0d12] p-4">
                          <p className="text-[10px] uppercase tracking-[0.25em] text-white/45">Top tracks in this cluster</p>
                          <div className="mt-3 space-y-2">
                            {selectedClusterTracks.slice(0, 3).map(track => (
                              <button
                                ...
                                className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left transition-colors ${selectedTrack?.name === track.name ? 'border-cyan-400/40 bg-cyan-400/10' : 'border-white/10 bg-white/5 hover:bg-white/8'}`}
                              >
                                <span className="min-w-0 truncate text-sm text-white">{track.name}</span>
                                <span className="ml-3 shrink-0 text-xs text-white/50">{track.release_year}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-cyan-500/10 to-fuchsia-500/10 p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Why it matters</p>
              ...
              {activeTab === 'galaxy' && (
                <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                  <div className="rounded-2xl border border-white/10 bg-[#090b10] p-4 sm:p-5">
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <div>
                        <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">2D manifold</p>
                        <h3 className="mt-1 text-xl font-semibold text-white">Music Galaxy Map</h3>
                      </div>
                      <div className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] uppercase tracking-[0.25em] text-white/55">
                        Click a dot to inspect
                      </div>
                    </div>

                    <div className="relative h-[420px] overflow-hidden rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_center,rgba(56,189,248,0.14),transparent_55%),linear-gradient(180deg,#10131a_0%,#07090d_100%)]">
                      ...
                      <div className="absolute left-4 top-4 rounded-2xl border border-white/10 bg-black/55 px-3 py-2 text-[10px] uppercase tracking-[0.25em] text-white/55 backdrop-blur">
                      ...
                      <div className="absolute bottom-4 left-4 rounded-2xl border border-white/10 bg-black/60 p-3 text-xs backdrop-blur">
                        <p className="mb-2 text-[10px] uppercase tracking-[0.25em] text-white/50">Cluster palette</p>
                        <div className="space-y-1.5">
                          {Object.entries(CLUSTER_NAMES).map(([key, meta]) => (
                            <button
                              key={key}
                              onClick={() => {
                                const nextCluster = Number(key);
                                setSelectedCluster(nextCluster);
                                const dominantTrackIndex = tracks.findIndex(track => track.cluster === nextCluster);
                                if (dominantTrackIndex >= 0) {
                                  setSelectedTrackIndex(dominantTrackIndex);
                                }
                              }}
                              className="flex items-center gap-2 text-left text-white/70 transition-colors hover:text-white"
                            >
                  ...
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Selected track</p>
                      <div className="mt-3 flex items-start justify-between gap-4">
                        <div>
                          <h4 className="text-lg font-semibold text-white">{selectedTrack?.name}</h4>
                          <p className="mt-1 text-sm text-white/60">{selectedTrack?.release_year}</p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#10131a] to-[#0b0d12] p-5">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Map takeaway</p>
                      <p className="mt-2 text-sm leading-6 text-white/70">
              ...
              {activeTab === 'comparison' && (
                <section className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Reference profile</p>
                    <h3 className="mt-2 text-xl font-semibold text-white">Madonna versus the pop field</h3>
                      ...
                      {[
                        ...
                        <div key={metric.label}>
                          <div className="mb-1 flex items-center justify-between text-xs text-white/60">
                            <span>{metric.label}</span>
                            <span className="text-white">{metric.value.toFixed(3)}</span>
                          </div>
                          <div className="h-2 overflow-hidden rounded-full bg-white/10">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-[10px] uppercase tracking-[0.3em] text-white/45">Diva DNA</p>
                        <h3 className="mt-2 text-xl font-semibold text-white">Closest pop neighbors</h3>
                      </div>
                      <div className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] uppercase tracking-[0.25em] text-white/55">
                        interactive list
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {divas
                        ...
                        .slice(0, 6)
                        .map(item => (
                          <div key={item.artists} className="rounded-xl border border-white/10 bg-[#10131a] p-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/8">
                            <div className="flex items-center justify-between gap-2">
                              <p className="truncate text-sm font-semibold text-white">{item.artists}</p>
                              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-white/5 text-white/50">
                                <ChevronRight className="h-3.5 w-3.5" />
                              </span>
                            </div>
                            <div className="mt-3 space-y-2 text-[10px] uppercase tracking-[0.2em] text-white/45">
            ...
            <div className="border-t border-white/10 px-5 py-3 text-xs text-white/45 sm:px-6">
