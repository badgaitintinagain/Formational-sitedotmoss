...
const AdTile: React.FC<AdTileProps> = ({ 
  ...
  return (
    <Tile 
      size="3x2" 
      label="Sponsored"
      bgClass="bg-palette-gray/40 border-white/10"
      bgImage={image}
    >
      <div className="flex flex-col items-center text-center px-3 py-2 bg-black/40 backdrop-blur-md rounded-none border border-white/10 shadow-xl transform transition-all duration-500 group-hover:scale-105 mx-1 mb-0.5">
        <h3 className="text-[10px] font-bold leading-tight mb-0.5 text-white/90">{title}</h3>
        <p className="text-[7px] leading-tight opacity-80 text-zinc-200 max-w-[140px]">{description}</p>
