...
export default function AdminCommentsPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="sticky top-0 z-50 border-b-2 border-foreground/25 bg-background/95 backdrop-blur-xl shadow-sm">
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-green-500/10 border-2 border-green-500/25 rounded-2xl p-4 text-center shadow-sm">
            ...
            <p className="text-xs text-foreground/70 mt-0.5">Approved</p>
          </div>
        </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all text-sm font-medium ${
                filter === item.key
                  ...
                  : 'bg-foreground/[0.08] text-foreground/70 border border-foreground/20 hover:bg-foreground/[0.12] hover:text-foreground'
              }`}
            >
              <item.icon size={15} />
              {item.label}
          ...
          {filteredComments.length === 0 ? (
            <div className="py-16 text-center">
              <div className="w-16 h-16 bg-foreground/[0.08] border-2 border-foreground/25 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm">
                <MessageSquare size={24} className="text-foreground/40" />
              </div>
            ...
            filteredComments.map((comment) => {
              ...
              return (
                <div
                  key={comment.id}
                  className={`bg-foreground/[0.08] backdrop-blur-sm border-2 border-foreground/25 rounded-2xl p-5 hover:bg-foreground/[0.12] transition-all shadow-sm hover:shadow-md ${isProcessing ? 'opacity-60 pointer-events-none' : ''}`}
                >
                  {/* Header row */}
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-accent-primary/15 flex items-center justify-center flex-shrink-0">
                  ...
                  {/* Post reference */}
                  <div className="text-xs text-foreground/60 mb-2 pl-[52px]">
                  ...
                  <div className="flex items-center gap-2 pl-[52px]">
                    ...
                    <button
                      ...
                      disabled={isProcessing}
                      className="flex items-center gap-1.5 px-3.5 py-2 bg-foreground/[0.08] border border-foreground/20 hover:bg-foreground/[0.12] text-foreground/65 hover:text-red-500 rounded-xl transition-colors text-sm font-medium ml-auto disabled:opacity-50"
                    >
                      <Trash2 size={15} />
                      Delete
                    </button>
