      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4 text-center">
            ...
            <p className="text-xs text-foreground/50 mt-0.5">Pending</p>
          </div>
          <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 text-center">
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{approvedCount}</p>
            <p className="text-xs text-foreground/50 mt-0.5">Approved</p>
          </div>
        </div>
          ...
          {([
            ...
            >
              ...
              <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${
                filter === item.key ? 'bg-accent-primary/20' : 'bg-foreground/10'
              }`}>{item.count}</span>
            </button>
          ))}
        </div>
          ...
          {filteredComments.length === 0 ? (
            <div className="py-16 text-center">
              <div className="w-16 h-16 bg-foreground/5 border border-foreground/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <MessageSquare size={24} className="text-foreground/25" />
              </div>
            ...
            filteredComments.map((comment) => {
              ...
              return (
                <div
                  key={comment.id}
                  className={`bg-foreground/5 border border-foreground/10 rounded-2xl p-5 hover:bg-foreground/8 transition-all ${isProcessing ? 'opacity-60 pointer-events-none' : ''}`}
                >
                  {/* Header row */}
                  ...
                  {/* Post reference */}
                  <div className="text-xs text-foreground/40 mb-2 pl-[52px]">
                  ...
                  <div className="flex items-center gap-2 pl-[52px]">
                    ...
                    <button
                      ...
                      disabled={isProcessing}
                      className="flex items-center gap-1.5 px-3.5 py-2 bg-foreground/5 hover:bg-foreground/10 text-foreground/50 hover:text-red-500 rounded-xl transition-colors text-sm font-medium ml-auto disabled:opacity-50"
                    >
                      <Trash2 size={15} />
                      Delete
                    </button>
